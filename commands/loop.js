...
module.exports = {
  name: "loop",
  description: "Включение или выключение режима повтора",
    ...
    try {
      ...
      let button = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          ...
          .setCustomId("queue"),
        new ButtonBuilder()
          .setLabel("Текущий трек")
          .setStyle(ButtonStyle.Secondary)
          .setCustomId("nowplaying"),
        new ButtonBuilder()
          .setLabel("Остановить повтор!")
      ...
      const embed = new EmbedBuilder()
        .setColor('#fc4e03')
        .setAuthor({
        name: 'Зацикливайте свои мелодии',
        ...
        .setDescription('**Делайте петли! Пусть музыка играет дальше и дальше. **')
      ...
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {
        ...
        col.on('collect', async (button) => {
          ...
          if (!queue1 || !queue1.playing) {
            await interaction?.editReply({ content: '⚠️ Музыка не обнаружена!!', ephemeral: true }).catch(e => { })
            await button?.deferUpdate().catch(e => {})
          }
          switch (button.customId) {
            case 'queue':
              const success = queue.setRepeatMode(2);
              interaction?.editReply({ content: `✅ Повтор очереди!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
              const success2 = queue.setRepeatMode(1);
              interaction?.editReply({ content: `✅ Повтор включен!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: '⚠️ Повтор уже отключен!!', ephemeral: true }).catch(e => { })
              }
              const success4 = queue.setRepeatMode(0);
              interaction?.editReply({ content: '▶️ Повтор отключен' }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
          }
        })
