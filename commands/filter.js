...
module.exports = {
  name: "equalizer",
  description: "Настрой музыку под себя",
  ...
  run: async (client, interaction) => {
    try {
      ...
      .setDescription('** Изучи ритм и прочувствуй настоящую магию звука!**')
...
if(!filters?.includes(button?.customId)) return
      ...
      if (!filtre) return interaction?.editReply({ content: '❌ Неверное имя', ephemeral: true }).catch(e => { })
     filtre = filtre?.toLowerCase()
      ...
      if (filters?.includes(filtre?.toLowerCase())) {
        ...
        embed?.setDescription(`❌ Настройка не найдена!!`.replace("{filters}", filters?.map(mr => `\`${mr}\``).join(", ")))
