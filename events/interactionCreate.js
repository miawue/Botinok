...
return interaction?.reply({ content: "Rate Limited.", ephemeral: true })
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            return interaction?.reply({ content: '🔴 Тариф ограничен'.replace("{channel_filter}", channel_filter), ephemeral: true }).catch(e => { })
            }
        }
            }
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `🔴 Для начала присоеденитесь к голосовому каналу!!`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `🔴 Должны быть в одном голосовом канале!!`, ephemeral: true }).catch(e => { })
            }
            }
            }
            ...
            return interaction?.reply({ content: `▶️ Отсутствующие права: **${props?.permissions?.replace("0x0000000000000020", "MANAGE GUILD")?.replace("0x0000000000000800", "SEND MESSAGES") || "SEND MESSAGES"}**`, ephemeral: true });
            }
            } catch (e) {
            return interaction?.reply({ content: `❌ Ошибка...\n\n\`\`\`${e?.message}\`\`\``, ephemeral: true });
            }
            }
            });
            });
